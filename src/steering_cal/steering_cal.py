...
def scan_callback(msg):
	...
	slope, intercept,inlier_points  = find_line(points) 
	rest = np.array([x for x in points if x not in inlier_points ])
	slope2, intercept2 ,inlier_points= find_line (rest)
...
def drive(multiplier , steering_angle ,speed ):
    ...
    print("Drive angle %d"%(steering_angle))
...
def do_calc(angles):
	...
	res = np.zeros(shape=(len(angles),3))
	...
	for angle in angles:
		...
		print("R is %s for vlaue %d"%(r,angle))
		gamma = calc_gamma(0.26,r)
		res[counter,0] = angle
		res[counter,1] = gamma
		res[counter,2] = np.rad2deg(gamma)
		if angle > 90 :
			gamma = - gamma


		print("Gamma is %s for vlaue %d"%(gamma , angle))
		print("Gamma in degree is %d"%(np.rad2deg(gamma)))
...
def main(args):
	...
	do_calc([0,30,60,90,120,150,179])
